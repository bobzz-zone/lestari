...
frappe.ui.form.on('SPK Lilin', {
	refresh: function(frm) {
		...
		if (cur_frm.is_new()){
			frappe.db.get_value("Employee", { "user_id": frappe.session.user }, ["name","id_employee"]).then(function (responseJSON) {
				...
				cur_frm.set_value("employee_id", responseJSON.message.id_employee);
	...
	from_spk: function(frm){
		
	},
	to_spk: function(frm){

	},
	get_spk: function(frm){
		frappe.call({
			method: 'get_spk_produksi',
			doc: frm.doc,
			// freeze: true,
			// freeze_message: __('Menggambil Data SPK'),
			callback: function(r) {
					frm.refresh()
			}
		});
