...
// 			this.child_results = r.message || [];
// 			if(this.doctype == "Material Request" && this.target.fields_dict.stock_entry_type.value === "Transfer Area"){
// 				this.child_results = this.child_results.filter(item => item.ordered_qty < item.qty)
// 			}
...
// 	// Define the function to get child results
...
// 		let is_desc = this.doctype == "Material Request" && this.target.fields_dict.stock_entry_type.value === "Transfer Area" ? " desc" : "" 
// 		console.log(filters)
// 		return frappe.call({
...
$.extend(lestari.ui.form.MultiSelectDialog.prototype, {
	...
	async get_child_result() {
		...
		console.log(filters)
		return frappe.call({
			method: "lestari.custom.custom_search.get_list",
			args: {
				doctype: this.child_doctype,
				filters: filters,
				fields: ['name', 'parent', ...this.child_columns],
