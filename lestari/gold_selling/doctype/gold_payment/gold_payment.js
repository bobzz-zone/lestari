...
frappe.ui.form.on('Gold Payment', {
			...
			if (frm.doc.unallocated_payment >0 || frm.doc.unallocated_idr_payment >0){
				...
				if(idr_need_to_deduct>0 && frm.doc.total_idr_advance>0){
					for (var i=frm.doc.invoice_advance.length;i>0 && idr_need_to_deduct>0;i--){
						var row = frm.doc.invoice_advance[i-1];
	...
	auto_distribute:function(frm){
		if (frm.doc.invoice_table==[] || frm.doc.customer_return==[]){
			...
			frappe.msgprint("Total IDR converted is "+ frm.doc.unallocated_idr_payment+" = "+idr_to_gold);
			...
			var unaloc = 0
			if (idr_to_gold>0){
				sisa_idr=parseFloat((idr_to_gold-(total_alo-saldo_gold))*frm.doc.tutupan).toFixed(0);
				...
				console.log("sisaidr1"+sisa_idr)

				frm.doc.unallocated_idr_payment=sisa_idr;
				cur_frm.refresh_field("unallocated_idr_payment")
				if (frm.doc.total_pajak>0 && frm.doc.fokus_piutang==1){
					...
					console.log("sisaidr2"+sisa_idr)
					console.log("idrneedto"+idr_need_to)
					for (var i = frm.doc.invoice_table.length - 1; i >= 0; i--) {
						var g = frm.doc.invoice_table[i];
						if (sisa_idr > g.outstanding_tax){
							g.tax_allocated=g.outstanding_tax;
			...
			if (saldo_gold <= total_alo){
				...
				unaloc=parseFloat(saldo_gold- total_alo).toFixed(3);
			...
			frappe.msgprint("Total Saldo Gold "+saldo_gold+" Telah ter alokasi "+ total_alo+" sehingga unallocated seharusnya "+unaloc);
