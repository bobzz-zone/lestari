...
class CustomerDeposit(StockController):
	...
	def on_cancel(self):
		...
		if self.deposit_type == "IDR":
			if (self.idr_left !=self.total_idr_deposit or self.total_idr_deposit==0):
				frappe.throw("Deposit ini sudah terpakai tidak bisa di cancel")
		else:
			if (self.gold_left != self.total_gold_deposit or self.total_gold_deposit==0):
				frappe.throw("Deposit ini sudah terpakai tidak bisa di cancel")
		self.make_gl_entries()
		if self.terima_barang==1 and self.is_convert==0:
			self.update_stock_ledger()
		...
		if self.is_convert==0:
			...
			if flt(self.gold_left) > 0 and self.deposit_type=="Emas":
				...
				gl[piutang_gold]={
											...
											"cost_center":cost_center,
											"debit":0,
											"credit":self.gold_left*self.tutupan,
											"account_currency":"GOLD",
											"debit_in_account_currency":0,
											"credit_in_account_currency":self.gold_left,
											#"against":"4110.000 - Penjualan - L",
											"voucher_type":"Customer Deposit",
											"voucher_no":self.name,
											#"remarks":"",
			...
			#untuk deposit IDR
			if self.idr_left and self.idr_left > 0 and self.deposit_type == "IDR":
				...
				gl[piutang_idr]={
										...
										"cost_center":cost_center,
										"debit":0,
										"credit":self.idr_left,
										"account_currency":"IDR",
										"debit_in_account_currency":0,
										"credit_in_account_currency":self.idr_left,
										#"against":"4110.000 - Penjualan - L",
										"voucher_type":"Customer Deposit",
										"voucher_no":self.name,
										#"remarks":"",
