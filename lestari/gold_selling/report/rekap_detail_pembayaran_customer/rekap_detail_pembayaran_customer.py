...
def execute(filters=None):
	...
	data = frappe.db.sql("""select cd.posting_date,cd.no_nota,cd.name as "Document No",sp.item,sp.qty,sp.rate,sp.amount,sp.keterangan from `tabStock Payment` sp left join `tabCustomer Deposit` cd on sp.parent=cd.name
							...
							union
							select gp.posting_date,"" as no_nota,gp.name as "Document No",sp2.item,sp2.qty,sp2.rate,sp2.amount,sp2.keterangan from `tabStock Payment` sp left join `tabGold Payment` gp on sp2.parent=gp.name
							...
							where gp2.docstatus=1 and gp2.customer="{customer}" and (gp2.posting_date < "{to_date}" or gp2.posting_date > "{from_date}"
